...
void libspdm_test_responder_digests_case10(void **state)
{
    ...
    for (uint8_t index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        spdm_context->local_context.local_cert_chain_provision[index] =
            &m_libspdm_local_certificate_chain[libspdm_get_hash_size(m_libspdm_use_hash_algo) *
                                               index];
        spdm_context->local_context
        .local_cert_chain_provision_size[index] = libspdm_get_hash_size(m_libspdm_use_hash_algo);
    }
